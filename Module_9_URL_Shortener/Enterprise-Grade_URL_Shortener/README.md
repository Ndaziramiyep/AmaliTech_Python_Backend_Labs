...
- **Auto-Fetched Link Previews** (url-preview): `title`/`description`/`favicon` left unset on create are fetched from the destination page in the background right after a short URL is created — never on the request path, and resilient by design: retry with backoff, a per-domain circuit breaker, an SSRF guard rejecting private/loopback/reserved addresses, and a Redis result cache; a slow, unreachable, or blocked destination just leaves those fields `null` rather than failing anything — see [`services/url-preview/README.md`](services/url-preview/README.md)
...
4. Right after that response is built, url-service queues `fetch_url_preview_task` (Celery, off the request path) for any of `title`/`description`/`favicon` the owner didn't already supply. The task calls **url-preview**, which SSRF-guards the destination URL, checks a Redis result cache, consults a per-domain circuit breaker, and — if the circuit is closed — fetches the page with retry/backoff and parses `<title>`/`og:description`/a favicon `<link>` out of the HTML. A slow, unreachable, or blocked destination just leaves those fields `null`; it never fails URL creation or retries indefinitely.
5. Visiting `/{short_code}/` (or a `custom_alias`) on url-service resolves the URL (cache first, then `url_db`; 404 if inactive or past `expires_at`), atomically increments `click_count`, and redirects (302) immediately. A background thread then geolocates the IP (via a free public API) and POSTs a click event to **analytics-service** (`short_code`, `owner_id`, referrer, user-agent, IP, city, country) — entirely off the request/response path, so a slow/unreachable analytics-service or geolocation API never delays the redirect.
...
7. Updating, deleting, or listing a URL checks the same token's `is_staff` claim against that Url's `owner_id` — the owner or an admin may proceed, anyone else gets a 403 (reads stay public — see [Role-Based Access](#-role-based-access)); write requests are also throttled per the token's `tier` claim (see [Rate Limiting](#rate-limiting)). Deleting cascades to analytics-service's click history for that code, via the same kind of background thread as click reporting.
8. Once a day, url-service's Celery Beat scheduler fires `archive_expired_urls`: every `Url` past its `expires_at` gets `is_archived=True`, `is_active=False`, and is evicted from the cache.
...
- **Resilient by design, not just fire-and-forget**: unlike the click-reporting hop, url-service's call to url-preview isn't a bare best-effort HTTP request — `PreviewService` (`apps/preview/api/services/preview_service.py`) layers a result cache, retry with backoff, and a Redis-backed per-domain `DomainCircuitBreaker` (`circuit_breaker.py`) that trips after `PREVIEW_CIRCUIT_FAILURE_THRESHOLD` failures and short-circuits further calls to that domain — with no network call at all — for `PREVIEW_CIRCUIT_OPEN_SECONDS`. Combined with an SSRF guard that resolves the destination hostname and rejects any private/loopback/link-local/reserved/multicast address before ever calling `requests.get`, since this is the one service in the platform that fetches arbitrary, attacker-influenceable URLs.
