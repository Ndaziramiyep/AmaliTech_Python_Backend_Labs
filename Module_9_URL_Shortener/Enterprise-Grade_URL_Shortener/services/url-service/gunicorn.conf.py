"""
Gunicorn tuning for url-service. Every knob is overridable via env var, set
in this service's own .env — gunicorn parses this file itself, before it
ever imports Config.wsgi/settings.py, so .env has to be loaded here
explicitly (django-environ, already a dependency) rather than relying on
Django to have done it first.

Bind is intentionally hardcoded, not env-driven: docker-compose.yml maps
host:8002 -> container:8000, so gunicorn must always listen on 8000 inside
...
# full core count. Threaded workers matter here specifically because
# RedirectUrlView spawns a background thread per redirect (click reporting +
# geolocation) — gthread keeps that from starving other requests on the
# same worker.
