...
2. **Build and start each service, in its own terminal**
   ...
   worker (plus Celery Beat, for url-service's nightly archive job), so each
   is fully self-contained. url-service still works fine if analytics-service
   isn't running yet — it just can't reach it to report clicks, and logs a
   warning each time instead of failing the redirect (see
...
2. **Start that service's own database and Redis** — using that service's own
   ...
   also starting the Django app in a container:
   ```bash
