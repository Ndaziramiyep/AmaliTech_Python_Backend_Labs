...
class CreateShortUrlView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "(register or log in first, then send the access token via "
            "the Authorize button as 'Bearer <your-access-token>')."
        ),
    )
...
class ResolveShortUrlView(APIView):
    ...
    def get(self, request, short_code):
        ...
        if original_url is None:
            raise Http404
        return Response({'short_url': short_code, 'original_url': original_url})
